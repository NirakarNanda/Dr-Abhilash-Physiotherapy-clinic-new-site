...
export function Resilience() {
  return (
    <section
      id="resilience"
      className="scroll-mt-20 border-t border-white/10 bg-coal"
    >
        ...
        <div className="mt-14 grid gap-5 md:mt-20 lg:grid-cols-2">
          {RESILIENCE.panels.map((panel, i) => (
            <Reveal key={panel.title} delay={0.08 * i} className="h-full">
              <article className="h-full overflow-hidden rounded-3xl border border-white/10 bg-coal-card transition-colors duration-500 hover:border-accent/30">
                <div className="relative">
                  ...
                  <div
                    ...
                    aria-hidden="true"
                  />
                  <span className="absolute left-6 top-6 rounded-full border border-white/20 bg-coal/60 px-4 py-1.5 text-[11px] font-display font-semibold uppercase tracking-label text-ivory backdrop-blur-sm">
                    {panel.tag}
                  </span>
                </div>
