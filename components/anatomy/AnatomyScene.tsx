...
export function AnatomyScene() {
  ...
  // Canvas background follows the theme: near-black in dark, light beige
  // in light. r3f re-applies <color> when its args change.
  const canvasBg = theme === "dark" ? "#0A0B0D" : "#F1ECE1";
  ...
  if (webglOk === false) {
    return (
      <div className="flex h-full w-full items-center justify-center bg-[var(--theme-bg)] transition-colors duration-300">
        <div className="w-[260px] text-center font-body">
          <p className="text-[11px] uppercase tracking-label text-[var(--theme-muted)]">
  ...
  return (
    ...
    >
      ...
      {/* Hemisphere fill: dark ground bounce in dark theme, warm beige
          bounce in light so the bones don't go muddy against the beige. */}
      <hemisphereLight
        args={
          theme === "dark"
            ? ["#FFFDF8", "#2A2C31", 0.4]
