  ...
  return (
    ...
    >
      <div ref={containerRef} className="flex flex-col gap-5">
        ...
        <h3
          className={`font-editorial ${titleSize} font-medium leading-[1.02] tracking-tight text-[var(--theme-ink)] transition-colors duration-300`}
        >
        ...
        <p className="max-w-xs font-body text-[15px] leading-relaxed text-[var(--theme-muted)] transition-colors duration-300">
          {section.description}
        </p>
        {section.keyStructures.length > 0 && (
          ...
          >
            {section.keyStructures.map((structure) => (
              <li
                key={structure}
                className="rounded-full border border-[var(--theme-chip-border)] bg-[var(--theme-chip-bg)] px-3 py-1 font-body text-[10px] uppercase tracking-[0.18em] text-[var(--theme-chip-text)] backdrop-blur-sm transition-colors duration-300"
              >
                {structure}
              </li>
            ))}
